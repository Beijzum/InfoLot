...
function getHistory(user) {
    db.collection("users").doc(user.uid).get()
        .then(userDoc => {
            ...
            let history = userDoc.data().reservations;
            console.log(history);

            // Get pointer the new card template
            let newcardTemplate = document.getElementById("savedCardTemplate");

            // Iterate through the ARRAY of reservations hikes (document ID's)
            history.forEach(reservationID => {
                db.collection("reserve_details").doc(reservationID).get()
                    .then(reservationDoc => {
                        if (reservationDoc) {
                            let parkingLotID = reservationDoc.data().parkingLotDocID;
                            console.log(parkingLotID);
                            db.collection("parkingLots").doc(parkingLotID).get()
                                .then(doc => {
